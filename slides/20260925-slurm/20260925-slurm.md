...
output for each node. And read some of `man sinfo` for more detail.
...
$ srun --pty --mem 4G --cpus-per-task 8 --time 10:00 bash -i
...
```bash
# Just for fun, ask for half a terabyte of RAM & 64 cores (just for 2 mins).
$ srun --pty --mem 512G --cpus-per-task 64 --time 2:00 bash -i
```
