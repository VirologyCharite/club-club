...
We will use Pixi: https://pixi.prefix.dev/latest/
...
Run the following, but _be careful_ to type it exactly. Make sure you
use `>>`.

```bash
$ echo export PIXI_CACHE_DIR=/tmp/pixi-cache-\$USER >> ~/.bashrc
$ source ~/.bashrc
```

If everything has worked, you should be able to execute
...
Using Pixi to install some programs
===

Make a Pixi workspace called "workspace":
